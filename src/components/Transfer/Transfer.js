...
class Transfer extends Component {
	...
	componentDidMount() {
		...
		let options = {
			url: 'http://3.83.91.13:8080/web_service_bank_pro/services/GetCustomerBalance?wsdl',
			method: 'POST',
			body: xml,
			headers: {
	...
	handleAddBalance = async e => {
		...
		if (addAmount === "") {
			...
			let options = {
				url: 'http://3.83.91.13:8080/web_service_bank_pro/services/AddBalance?wsdl',
				method: 'POST',
				body: xml,
				headers: {
					'Content-Type': 'text/xml;charset=utf-8',
	...
	handleTransfer = async e => {
		...
		if (receiverAccount === "") {
			...
			let validationOptions = {
				url: 'http://3.83.91.13:8080/web_service_bank_pro/services/GetAccountNumber?wsdl',
				method: 'POST',
				body: validationXML,
				headers: {
					'Content-Type': 'text/xml;charset=utf-8',
				...
				if (!error && response.statusCode === 200) {
					...
					xml2js.parseString(resultResponse, xmlOptions, (err, res) => {
						...
						if (validationResult["status"] === "200") {
							...
							let options = {
								url: 'http://3.83.91.13:8080/web_service_bank_pro/services/Transfer?wsdl',
								method: 'POST',
								body: xml,
								headers: {
									'Content-Type': 'text/xml;charset=utf-8',
